...
export default function MaterialsPage() {
  ...
  const { data: currentMaterial, isLoading: contentLoading } = useMaterialByChapterId(
    typeof selectedSection === 'number' ? selectedSection : undefined
  );

  const [signsCategory, setSignsCategory] = useState('all');
  const [signsPage, setSignsPage] = useState(1);
  const { data: signsData, isLoading: signsLoading } = useTrafficSigns({
  ...
  }, [sectionOrder, search, materialsList, t]);
                ...
                <GlassCard className="p-2 space-y-1 border border-border/80 bg-black/75 max-h-[calc(100vh-300px)] overflow-y-auto custom-scrollbar">
                  ...
                  >
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0 ${
                      selectedSection === 'signs' ? 'bg-orange-500 text-black' : 'bg-white/10 text-muted-foreground'
                    }`}>
                      <Signpost className="w-3 h-3" />
                    </div>
                    <span className="truncate text-left line-clamp-2 leading-tight">
                      {isSq ? 'Shenjat e Trafikut' : 'Traffic Signs'}
                    </span>
                    {selectedSection === 'signs' && <ChevronRight className="w-4 h-4 ml-auto text-orange-400 flex-shrink-0" />}
                  </Button>
                  ...
                  <div className="flex flex-col gap-6 mb-8">
                    <div className="space-y-3 border-b border-border/40 pb-8">
                      ...
                      <p className="text-sm text-muted-foreground max-w-2xl leading-relaxed">
                        {isSq 
                          ? 'Mësoni të gjitha shenjat e trafikut të ndara sipas kategorive. Njohja e tyre është thelbësore për sigurinë në rrugë.' 
                          : 'Learn all traffic signs divided by categories. Knowing them is essential for road safety.'}
                      </p>
                    </div>
                        ...
                        {signsData?.signs.map((sign) => (
                          ...
                          >
                            ...
                            <div className="p-4 flex-1 flex flex-col">
                              <h3 className="font-semibold text-sm mb-1 line-clamp-2 group-hover:text-orange-400 transition-colors">
                                {sign.name}
                              </h3>
                    ...
                    {/* Signs Pagination */}
                    {signsData && signsData.total > 50 && (
                      <div className="flex justify-center gap-2 mt-8 pt-4 border-t border-border/20">
                        <Button
                          variant="outline"
                        ...
                        >
                          {isSq ? 'E mëparshmja' : 'Previous'}
                        </Button>
                        <div className="flex items-center px-4 text-sm text-muted-foreground bg-black/20 rounded-md border border-white/5">
                          {isSq ? 'Faqja' : 'Page'} {signsPage} / {Math.ceil(signsData.total / 50)}
                        </div>
                        <Button
                          variant="outline"
